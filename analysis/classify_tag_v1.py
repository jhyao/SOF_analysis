...
if __name__ == '__main__':
    clifer = TagClassifier(from_db=False, from_api=True, update=False, min_weight=0.1, save_db=False, related_cache=False)
    # TagsCDN.clear_tag_category()
    tags = Tag.select().where(Tag.count >= 100).order_by(-Tag.count).execute()
    for i, tag in enumerate(tags):
    # for i, tag in enumerate(QuestionsCDN.tags_have_questions(min_questions=100)):
        logger.info(f'count {i}')
        c = clifer.classify_tag(tag.name)
    # clifer.classify_tag('javascript')